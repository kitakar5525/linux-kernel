...
static struct gmin_subdev *gmin_subdev_add(struct v4l2_subdev *subdev)
{
	...
	gmin_subdevs[i].gpio0 = gpiod_get_index(dev, "cam_gpio0", 0, GPIOD_OUT_LOW);
	gmin_subdevs[i].gpio1 = gpiod_get_index(dev, "cam_gpio1", 1, GPIOD_OUT_LOW);
		...
		int *result)
{
	...
	obj = acpi_evaluate_dsm_typed(handle, oemvars_guid, 0, 0, NULL,
						ACPI_TYPE_PACKAGE);
	...
	for (i = 0; i < obj->package.count; i += 2) {
		...
		if (item->type == ACPI_TYPE_STRING &&
				!strcmp(item->string.pointer, var)) {
			union acpi_object *str_item;

			str_item = &(obj->package.elements[i + 1]);
			ret = kstrtoint(str_item->string.pointer, 0, &res);
		}
	}
