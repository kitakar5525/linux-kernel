...
static void cio2_bridge_create_fwnode_properties(struct cio2_sensor *sensor)
{
	...
	sensor->dev_properties[0] = PROPERTY_ENTRY_U32(
                                        kasprintf(GFP_KERNEL, "clock-frequency"),
                                        sensor->ssdb.mclkspeed);
	sensor->dev_properties[1] = PROPERTY_ENTRY_U8(
                                        kasprintf(GFP_KERNEL, "rotation"),
                                        sensor->ssdb.degree);

	sensor->ep_properties[0] = PROPERTY_ENTRY_U32(
                                        kasprintf(GFP_KERNEL,"bus-type"), 5);
	sensor->ep_properties[1] = PROPERTY_ENTRY_U32_ARRAY_LEN(
                                        kasprintf(GFP_KERNEL, "data-lanes"),
                                        sensor->data_lanes,
                                        sensor->ssdb.lanes);
	sensor->ep_properties[2] = PROPERTY_ENTRY_REF_ARRAY(
                                        kasprintf(GFP_KERNEL, "remote-endpoint"),
                                        sensor->local_ref);
...
static void cio2_bridge_create_connection_swnodes(struct cio2_bridge *bridge, struct cio2_sensor *sensor)
{
	...
	nodes[SWNODE_SENSOR_PORT] = NODE_PORT(kasprintf(GFP_KERNEL, "port0"),
					      &nodes[SWNODE_SENSOR_HID]);
	nodes[SWNODE_SENSOR_ENDPOINT] = NODE_ENDPOINT(kasprintf(GFP_KERNEL, "endpoint0"),
						      &nodes[SWNODE_SENSOR_PORT],
						      sensor->ep_properties);
	nodes[SWNODE_CIO2_PORT] = NODE_PORT(
                                        kasprintf(GFP_KERNEL, "port%u", sensor->ssdb.link),
					&bridge->cio2_hid_node);
	nodes[SWNODE_CIO2_ENDPOINT] = NODE_ENDPOINT(
                                        kasprintf(GFP_KERNEL, "endpoint0"),
...
static int cio2_bridge_connect_sensors(struct cio2_bridge *bridge)
{
	...
	for (i = 0; i < ARRAY_SIZE(cio2_supported_devices); i++) {
		...
		for_each_acpi_dev_match(adev, this_device, NULL, -1) {
			...
			dev_info(&bridge->cio2->dev, "Found supported sensor %s\n", acpi_dev_name(adev));
