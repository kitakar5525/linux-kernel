/****************************************************************************
 *
 * Driver for the XMM2230 spi modem.
 *
...
static u64 xmm2230_dmamask = 0xffffffffffffffff;
 ...
 *	@clear: bits to clear
 *
 *	The XMM2230 only supports DTR and RTS. Set them accordingly
 ...
 *	FIXME: endianness?
 */
static void ifx_spi_setup_spi_header(struct ifx_spi_device *ifx_dev, int tx_count)
{
	unsigned char *txbuffer;
	txbuffer = ifx_dev->tx_buffer;

	*(u16 *)(txbuffer) = tx_count;

	if (ifx_dev->frame_len_flag_rm)
		*(u16 *)(txbuffer+2) = IFX_SPI_PAYLOAD_SIZE_RM;
	else
		*(u16 *)(txbuffer+2) = IFX_SPI_PAYLOAD_SIZE;

	txbuffer[1] |= (ifx_dev->spi_more << IFX_SPI_MORE_BIT) & IFX_SPI_MORE_MASK;
}
...
static int ifx_spi_prepare_tx_buffer(struct ifx_spi_device *ifx_dev)
{
	...
	if (!ifx_dev->spi_slave_cts) {
		...
		if (queue_length != 0) {
			...
			if (ifx_dev->frame_len_flag_rm)
				temp_count = min(queue_length, IFX_SPI_PAYLOAD_SIZE_RM);
			else
	...
	ifx_spi_setup_spi_header(ifx_dev,
					tx_count-IFX_SPI_HEADER_OVERHEAD);
	/* swap actual data in the buffer */
	if (ifx_dev->frame_len_flag_rm) {
		ifx_dev->swap_buf((ifx_dev->tx_buffer), tx_count,
		&ifx_dev->tx_buffer[IFX_SPI_TRANSFER_SIZE_RM]);
	} else {
		ifx_dev->swap_buf((ifx_dev->tx_buffer), tx_count,
...
/**
 *	ifx_spi_ioctl		-	line discipline ioctl
 *	@tty: our tty device
 *	@cmd: command to chang the length of frame
 *
 *	Chang the length of frame according downloading the 2230 fw or
		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case SPI_IOC_FRAME_LEN_DOWNLOAD:
		ifx_dev->frame_len_flag_rm = false;
		break;
			 ...
			 int count)
{
	...
	if (!ifx_dev->frame_len_flag_rm && kfifo_len(&ifx_dev->tx_fifo) != 0)
		return 0;
...
static int ifx_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	if (srdy)
		dev_err(&ifx_dev->spi_dev->dev, "srdy is high");
	...
	/* set flag to allows big data transfer for download modem fw*/
...
static void ifx_spi_complete(void *ctx)
{
	...
	if (!ifx_dev->spi_msg.status) {
		...
		if (ifx_dev->frame_len_flag_rm) {
			...
			(ifx_dev->rx_buffer + IFX_SPI_HEADER_OVERHEAD),
			 actual_length,
			 &ifx_dev->rx_buffer[IFX_SPI_TRANSFER_SIZE_RM]);
		} else {
			ifx_dev->swap_buf(
			(ifx_dev->rx_buffer + IFX_SPI_HEADER_OVERHEAD),
			 actual_length,
...
static void ifx_spi_io(unsigned long data)
{
	...
	if (!test_and_set_bit(IFX_SPI_STATE_IO_IN_PROGRESS, &ifx_dev->flags) &&
		...
		if (ifx_dev->frame_len_flag_rm)
			ifx_dev->spi_xfer.len = IFX_SPI_TRANSFER_SIZE_RM;
		else
 ...
 *
 *	Probe for a 2230 modem on SPI bus. Perform any needed device and
 *	GPIO setup.
 *
 *	FIXME:
...
static int ifx_spi_spi_probe(struct spi_device *spi)
{
	...
	ifx_dev->spi_more = 0;
	ifx_dev->spi_slave_cts = 0;
	ifx_dev->spi_dev->dev.dma_mask = &xmm2230_dmamask;
...
MODULE_AUTHOR("Intel");
MODULE_DESCRIPTION("xmm2230 spi driver");
MODULE_LICENSE("GPL");
MODULE_INFO(version, "0.1-xmm2230");
